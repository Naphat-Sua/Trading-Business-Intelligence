...
class V4(QCAlgorithm):
    ...
    def calculate_position_size(self):
        ...
        risk_per_trade = 0.02  # Lower risk per trade for improved consistency
    ...
    def calculate_take_profit(self, entry_price, is_long):
        atr_value = self._aatr.current.value
        tp_multiplier = 3  # Adjusted for better reward-to-risk ratio
    ...
    def on_data(self, data: Slice):
        ...
        # Enhanced entry conditions with stricter trend alignment
        ...
        allow_entry_long = (
            adx > 25 and  # Adjusted ADX threshold
        ...
        if curr_qty != 0:
            if self.bar_count - self.entry_bar >= 48:  # Extended holding period
                self.liquidate(tag="Time Exit")
                return

            # Update trailing stop
            if curr_qty > 0:
                self.stop_loss_price = max(self.stop_loss_price, close - atr_val * 1.5)
            else:
                self.stop_loss_price = min(self.stop_loss_price, close + atr_val * 1.5)

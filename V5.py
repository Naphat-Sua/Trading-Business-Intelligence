...
class V5(QCAlgorithm):
    # Rooooooooooooooooooolling coaster
    def initialize(self):
    ...
    def calculate_position_size(self):
        ...
        risk_per_trade = 0.015  # Reduced risk per trade for steadier growth
    ...
    def calculate_take_profit(self, entry_price, is_long):
        atr_value = self._aatr.current.value
        tp_multiplier = 4  # Higher multiplier for improved reward:risk ratio
    ...
    def on_data(self, data: Slice):
        ...
        if curr_qty != 0:
            ...
            if curr_qty > 0:
                self.stop_loss_price = max(self.stop_loss_price, close - atr_val * 1.2)
            else:
                self.stop_loss_price = min(self.stop_loss_price, close + atr_val * 1.2)
